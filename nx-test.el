...
(defun nx-test-setup ()
  ...
  (list
   (nx :root (ht (:_nx/id 'root)) (list (nx :child (ht (:_nx/id 'child) (:prop "value")))))
   (nx :root (ht (:_nx/id 'root)) (list (nx :child (ht (:_nx/id 'child) (:prop "new-value")))))))
...
;;       ;; (should (equal (ht-get (caddr (car diffs)) :id) "child-2"))
;;       (should (nx-test-equal applied-tree tree2))
;;       (should (not (nx-test-equal tree1 applied-tree))))))
